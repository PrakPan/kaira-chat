...
export default function Packages(props) {
  ...
  return (
    <div className="relative flex flex-col gap-3">
      ...
      <div className="w-fit relative font-bold text-[30px] md:text-[40px]">
        <Image
...
const Navigation = ({ activeTab, setActiveTab, domestic, setDomestic }) => {
  ...
  const handleMenu = (domestic) => {
    if (activeTab === "Religious" && !domestic) {
      setShowMenu(false);
      return;
    }
  ...
  return (
    <div className="relative">
      ...
      {showMenu && (
        <div
          ref={menuRref}
          className="z-50 md:px-5 md:py-3 flex flex-col gap-3 absolute right-0 top-[100%] bg-white drop-shadow-2xl p-3 rounded-lg cursor-pointer"
        >
          <div
            onClick={() => handleMenu(false)}
            className="border-1 px-3 py-1 rounded-full"
          >
            International
          </div>
          <div
