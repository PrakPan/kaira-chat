...
const transformItineraryData = (apiItineraries) => {
  return apiItineraries.map((item) => {
    ...
    const price = `₹${Math.round(payment.discounted_cost).toLocaleString(
      "en-IN"
    )}`;

    // Calculate original price (assuming 10% discount for demo)
    const originalPrice = `₹${Math.round(
      payment.discounted_cost * 1.1
    ...
    if (summary.Activities?.count > 0) {
      highlights.push(
        `${summary.Activities.count} exciting activities included`
      );
    }
    if (summary.Stays?.count > 0) {
      highlights.push(
        `${summary.Stays.count} premium stays. +${Math.max(
          0,
          summary.Stays.count - 2
        )}`
      );
    }
    ...
    return {
      ...
      image: images,
      highlights:
        highlights.length > 0 ? highlights : ["Curated by " + item.curated_by],
      pax: payment.pax,
    };
  });
};
        ...
        {itineraries?.[0]?.id === 1 && (
          <div className="text-center mb-12 lg:mb-16">
            ...
            <p className="text-gray-600 text-base max-w-2xl mx-auto leading-6">
              These itineraries are traveler favorites, packed with iconic
              sights, hidden gems, and unforgettable moments in every stop.
            </p>
          </div>
        )}
              ...
              <SwiperSlide key={itinerary.id} className="flex justify-center">
                <ItineraryCard
                  ...
                  onClick={() => {
                    console.log(
                      itinerary,
                      `Clicked on ${itinerary.name || itinerary.title}`
                    );
