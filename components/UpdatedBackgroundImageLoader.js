...
const SaifBackgroundImageLoader = (props) => {
  ...
  useEffect(() => {
    ...
    const img = new Image();
    img.src = `${imgUrlEndPoint}/${Buffer.from(imageRequest).toString(
      "base64"
    )}`;
    img.decode().then(() => {
      ...
      <>
        ...
        >
          <BackgroundImageContainer
            style={{
              backgroundImage: props.filter
                ? props.filter +
                  `,url(${`${imgUrlEndPoint}/${Buffer.from(
                    smallImageRequest
                  ).toString("base64")}`})`
                : props.position
                ? `linear-gradient(180deg, rgba(0, 0, 0,0) 0%, rgba(0, 0, 0, 0.8) 100%), url(${`${imgUrlEndPoint}/${Buffer.from(
                    smallImageRequest
                  ).toString("base64")}`})`
                : `linear-gradient(180deg, rgba(0, 0, 0,0) 0%, rgba(0, 0, 0, 0.8) 100%), url(${`${imgUrlEndPoint}/${Buffer.from(
                    smallImageRequest
        ...
        <FullContainer
          ...
          style={{
            backgroundImage: props.filter
              ...
              : `url(${`${imgUrlEndPoint}/${Buffer.from(smallImageRequest).toString(
                  "base64"
                )}`})`,
            width: props.width ? props.width : "100%",
            maxWidth: "100%",
            height: props.height ? props.height : "100%",
            display: fullLoaded ? "none" : "flex",
            backgroundRepeat: "no-repeat",
        ...
        <FullContainer
          ...
          style={{
            backgroundImage: props.filter
              ...
              : `url(${`${imgUrlEndPoint}/${Buffer.from(imageRequest).toString(
                  "base64"
                )}`})`,
            width: props.width ? props.width : "100%",
            maxWidth: "100%",
