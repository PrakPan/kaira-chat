...
const AirbnbCalendar = (props) => {
  ...
  const [currentMonth, setCurrentMonth] = useState(
    props.date.year && props.date.month
      ? new Date(props.date.year, props.date.month, 1)
  ...
  const handleDateClick = (date) => {
    ...
    if (!selectedDates.start) {
      ...
      setTripDuration(1);
      setSelectionMessage("Now select your end date to continue");
      return;
    }
    ...
    if (!selectedDates.end) {
      if (dateTime === startTime) {
        ...
        setTripDuration(1);
        setSelectionMessage("Select your start date");
      } else if (dateTime > startTime) {
        setSelectedDates({ start: selectedDates.start, end: date });
        setTripDuration(
          Math.ceil((date - selectedDates.start) / (1000 * 60 * 60 * 24)) + 1
        );
        setSelectionMessage("");
      } else {
        setSelectedDates({ start: date, end: selectedDates.start });
        setTripDuration(
          Math.ceil((selectedDates.start - date) / (1000 * 60 * 60 * 24)) + 1
        );
        setSelectionMessage("");
      }
      return;
  ...
  const renderMonthGrid = (days) => (
    <div>
      <div className="grid grid-cols-7 gap-1 mb-[2px]">
        {dayNames.map((day) => (
          <div key={day} className="text-[10px] font-medium text-center ">
            {day}
          </div>
        ))}
      </div>
    ...
    ${
      date && isDateInRange(date, selectedDates)
        ? "bg-primary-jasmineWhite"
        : ""
    }
    ${
      date && isDateInHoverRange(date, selectedDates, hoveredDate)
        ? "bg-gray-200"
        : ""
    }
    ${
      date && isDateRangeStart(date, selectedDates)
        ? "bg-primary-jasmineWhite rounded-l-full"
        : ""
    }
    ${
      date && isDateRangeEnd(date, selectedDates)
        ? "bg-primary-jasmineWhite rounded-r-full"
        : ""
    }
    ${
      date && isDateInRange(date, selectedDates) && isRowStart
        ? "rounded-l-full"
        : ""
    }
    ${
      date && isDateInRange(date, selectedDates) && isRowEnd
        ? "rounded-r-full"
        : ""
    }
    ${
      date && isDateInHoverRange(date, selectedDates, hoveredDate) && isRowStart
        ? "rounded-l-full"
        : ""
    }
    ${
      date && isDateInHoverRange(date, selectedDates, hoveredDate) && isRowEnd
        ? "rounded-r-full"
        : ""
    }
  `}
      ...
      ${
        isDateSelected(date, selectedDates)
          ? "bg-primary-yellow text-pureBlack shadow-md"
          : isDateInRange(date, selectedDates)
          ? "hover:bg-gray-200 bg-primary-jasmineWhite"
          : isDateInHoverRange(date, selectedDates, hoveredDate)
    ...
    <div className="space-y-8">
      <div className="grid grid-cols-4 gap-4">
        {getNext12Months().map((month, index) => (
          <button
            ...
            className={`flex flex-col  justify-start p-[10px] text-start text-[14px] gap-xs rounded-xl border-sm transition-all ${
              currentMonth.getMonth() === month.monthIndex &&
              currentMonth.getFullYear() === month.year
      ...
      <div className="flex items-center justify-between  bg-[#F0F0F0] px-[8px] py-[10px] rounded-md text-[14px">
        ...
        <div className="flex items-center space-x-4">
          ...
          <span className="flex w-[87px] px-[10px] py-[6px] justify-center items-center gap-[10px] rounded-[40px] border border-[var(--Text-Colors-Stroke,#E5E5E5)] bg-[var(--text-colors-text-white,#FFF)]">
            {tripDuration}
          </span>
      ...
      <div className="flex flex-col gap-5">
        <div className="flex flex-col gap-[20px] w-full">
              ...
              <div className="flex bg-gray-100 rounded-full px-[12px] py-[6px] w-fit mx-auto">
                {["fixed", "flexible", "anytime"].map((type) => (
                  <button
                    ...
                    className={`px-[24px] py-[4px] rounded-full text-[14px] font-500 transition-all ${
                      dateType === type
                        ? "bg-pureBlack text-text-white shadow-sm"
          ...
          <div className="flex flex-col gap-1">
            {dateType === "fixed" && selectionMessage && (
              <p className="text-sm italic text-red-600 text-right mr-5">
                {selectionMessage}
              </p>
            )}
            <div className="flex justify-end">
              <div className="flex gap-[20px]">
                ...
                <MediumIndigoButton
                  ...
                  className="px-[26px] py-[8px] bg-gray-900 text-white font-medium rounded-[8px] hover:bg-gray-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {loading ? (
                    <PulseLoader
                      style={{
