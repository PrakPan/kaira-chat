...
const Booking = (props) => {
    ...
    const EXPERIENCE_FILTERS = [ 'Isolated',  'Romantic',  'Heritage',  'Spiritual',  'Art and Culture', 'Shopping', 'Adventure and Outdoors', 'Nature and Retreat', 'Nightlife and Events', 'Science and Knowledge'];
        ...
        axiosaxtivitiesinstance.post("/", 
        {
            ...
            if(res.data.length){
                ...
                for(var i = 0; i<res.data.length; i++){
                    ...
                    options.push(<Poi  _updatePoiHandler={_updatePoiHandler} _openPoiModal={_openPoiModal} data={res.data[i]} tailored_id={props.tailored_id} updateLoadingState={updateLoadingState} itinerary_id={props.selectedBooking ? props.selectedBooking.itinerary_id : ''}></Poi>)
    ...
    const _updatePoiHandler = (poi) => {
            ...
            axiositineraryeditinstance.post("/", 
            {
  ...
  return(
      <div>
        <Modal   id="bookingedit-modal" show={props.showPoiModal}  size="xl"  onHide={props.setHidePoiModal} style={{padding: "0"}}>
           {/* <Modal.Header>2</Modal.Header> */}
            <Modal.Body style={{padding: "0rem 0",  borderStyle: "solid", borderColor: "#F7e700", borderWidth: "0.5rem", borderRadius: '16px', backgroundColor: 'white', }} >
            <div style={{padding: '0.25rem'}}>
               ...
               <GridContainer style={{clear: 'right'}}>
                <LeftSideBar selectedPoi={props.selectedPoi} replacing={props.selectedPoi? props.selectedPoi.name: ''}  setHideBookingModal={props.setHidePoiModal}></LeftSideBar>
               ...
               {noResults ? <p  className='font-lexend text-center' >Oops, we couldn't find what you were searching but we are already adding new activities to our database everyday!</p>  : null}
                ...
                </PoiDetailsContainer>:null}
               </GridContainer>
               {!isPageWide ? <CurrentlyReplacing selectedPoi={props.selectedPoi} replacing={props.selectedPoi ? props.selectedPoi.name : ''}></CurrentlyReplacing> : null}
               </div>
            </Modal.Body>
