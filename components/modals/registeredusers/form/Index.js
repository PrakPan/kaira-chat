...
const Enquiry = (props) => {
    ...
    for(var i = 0 ; i < props.registered_users.length ; i++){
        emails.push(
         ...
         status.push(
            <Text className="text-center bordr" style={{borderStyle: i ?  'solid solid solid none' : 'solid solid solid none', borderWidth: '1px', padding: '1rem', color: props.registered_users[i].payment_status ? 'green' : 'orange',  borderRadius: '0 10px 10px 0'}}>{!props.registered_users[i].payment_status ? 'Invited' : 'Paid'}</Text>
         )
        //  pax.push(
        //     <div>
