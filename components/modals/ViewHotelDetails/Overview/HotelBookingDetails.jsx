...
const HotelBookingDetails = (props) => {
  ...
  useEffect(() => {
    calculateVisibleFacilites(props?.data?.facilities, false)
  }, [])
  ...
  const viewAllGalleryLink = props.images && props.images.length && ImagesLoaded[0] ? (
    <div className="absolute top-0 left-0 right-0 bottom-0 flex align-items-center justify-center bg-trans-black_70"
    >
      <span onClick={() => props._setImagesHandler(props.images)} className="font-600 text-sm-md leading-sm-md  border-solid border-b-sm border-text-white text-white cursor-pointer"> Show all photos</span>
    </div>
  ) : null;
    ...
    <Container>
      ...
      <div className="overflow-y-auto" ref={scrollableTabRef} style={{ 'height': `calc( 100vh - 195px` }}>
        ...
        <div className="flex flex-col">
          <div ref={stickyTabsRef} className="sticky top-0 z-10 bg-[#fafaf5] border-b-sm border-solid border-[#ececec]">
            <ScrollableMenuTabs
              classStyle="w-100"
              items={items}
                      ...
                      <div className="text-sm-md font-400 leading-xl text-[#445069]"> {dateFormat(props?.data?.check_in.date)}
                        {props?.data?.check_in.begin_time && (
                          <>
                            |
                            {getHumanTime(
                              dateFormat(props?.data?.check_in.begin_time)
                            )}
                          </>
                        )}
                      </div>
                      ...
                      <div className="text-sm-md font-400 leading-xl text-[#445069]">
                        ...
                        {props?.data?.check_out.time && (
                          <>
                            |
                            {getHumanTime(
                              dateFormat(props?.data?.check_out.time)
                            )}
                          </>
                        )}{" "}
                      </div>
                      ...
                      <div>
                        {props?.data?.rating  && (
                          <div className="gap-1 flex flex-column text-sm-md text-[#445069] font-[400]">
                            ...
                            {props?.data?.user_ratings_total > 0 || props?.data?.user_ratings_total > 0 && (
                              <div className="underline">
                                ({props?.data?.user_ratings_total || props?.data?.user_ratings_total} Reviews)
                              </div>
                            )}
                          </div>
                        )}
