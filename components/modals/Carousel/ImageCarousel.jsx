        ...
        <div onMouseEnter={() => setMouseHovered(true)} onMouseLeave={() => setMouseHovered(false)} className="relative w-full h-full">
            ...
            <div className='absolute bottom-4 left-1/2 -translate-x-1/2 z-50 flex flex-row gap-2 items-center'>
                {visibleDots.map((index) => {
                    const isPrevious = (currentIndex === 0 && index === images.length - 1) ||
                        (index === currentIndex - 1);
                    const isNext = (currentIndex === images.length - 1 && index === 0) ||
                        (index === currentIndex + 1);
                    ...
                    return (
                        <div
                            ...
                            className={`
                                ...
                                ${isCurrent ? 'scale-100 bg-[#F7E700] border border-black' : ''}
                                ${!isPrevious && !isNext && !isCurrent ? 'scale-50 bg-gray-200' : ''}
                            `}
                        ></div>
                    );
                })}
