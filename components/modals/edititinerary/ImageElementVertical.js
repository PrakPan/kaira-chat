...
const Poi = (props) =>{
    ...
    return(
        <Container>
            {/* <ImageLoader url={props.image} dimensions={{width: 400, height: 400}} width="40%" margin="auto" ></ImageLoader> */}
            <ImgContainer className="center-div" onClick={() => setSelectedState({...selectedState, one: !selectedState.one})} style={{backgroundImage : `url(${props.image})`}} onMouseEnter={() => setHoverState({...hoverState, one: true})} onMouseLeave={()=> setHoverState({...hoverState, one: false})}>
                        {selectedState.one  ? 
            ...
            <div style={{padding: "0 1rem"}}>
                <PoiName className=""><b>{props.name}</b></PoiName>
                {/* <PoiDescription>{props.text}</PoiDescription> */}
            </div>
        </Container>
    );
}
