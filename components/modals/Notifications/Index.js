...
const Enquiry = (props) => {
        ...
        if(props.token){
                ...
                for(var i=0; i<props.notifications.length; i++){
                    ...
                    notificationsarr.push(
                            <Notification  _deleteNotificationHandler={() =>  props._deleteNotificationHandler(id)} id={props.notifications[i].id} heading={props.notifications[i].heading} text={props.notifications[i].text} cta_link={props.notifications[i].cta_link}  status={props.notifications[i].status}></Notification>
                    );
                }
    ...
    const _clearAllHandler = () => {
        setNotificationsArr([]);
        axiosnotificationsinstance.delete("",  {headers: {
