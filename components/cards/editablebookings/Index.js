...
const getDate = (date) => {
 ...
 const _handleTaxiSelection = () => {
    ...
    props._selectTaxiHandler(null, props.booking_id, props.booking_name, props.booking_type, props.itinerary_id, props.tailored_id, props.itinerary_name, props.taxi_type, props.transfer_type, props.city_id, props.destination_city_id, props.duration, props.check_in );
}
     if(isPageWide)
    return(
        <Container className="border-thin" style={{borderColor: props.is_selected ? "#f7e700" : '#e4e4e4', borderWidth: '2px'}}>
            ...
            <div>
                {/* <Accordion> */}
                <div style={{padding: "0.5rem 0.5rem 0.5rem 0.5rem", height: '100%'}}>
...
<Container className="border-thin" style={{borderColor: props.is_selected ? "#f7e700" : '#e4e4e4', borderWidth: '2px'}}>
            <ImageContainer duration={props.duration} tag={props.tag} are_prices_hidden={props.are_prices_hidden} _setImagesHandler={props.setImagesHandler} check_in={props.check_in} check_out={props.check_out} city={props.city} price={props.price} type={props.type} setShowBookingModal={props.setShowBookingModal} heading={props.heading} details={props.details} blur={props.blur} images={props.images} setImagesHandler={props.setImagesHandler}></ImageContainer>
            ...
            <div style={{padding: "0.5rem 0.5rem 3rem 0.5rem"}}>
                ...
                {props.type === "Accommodation" ? 
                <div>
                {props.check_in ? <Detail  className='font-opensans'>{ 'Check in: ' + getHumanDate((props.check_in.slice(5)+"-"+props.check_in.substring(0,4)).replaceAll('-','/')) + " "+ props.check_in.substring(0,4)}</Detail>:null}
                {props.check_out ? <Detail  className='font-opensans'>{ 'Check out: ' + getHumanDate((props.check_out.slice(5)+"-"+props.check_out.substring(0,4)).replaceAll('-','/')) + " "+ props.check_out.substring(0,4) }</Detail>:null}
