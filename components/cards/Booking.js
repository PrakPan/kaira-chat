...
import { faMapMarker, faStar, faImages} from '@fortawesome/free-solid-svg-icons';
...
const Booking = (props) =>{
    ...
    for(var i=0; i<props.details.length; i++){
        detailsarr.push(
            <li key={props.details[i]} className={props.blur ? 'blurry-text' : ''} style={{fontSize: "0.75rem", margin: "0.5rem 0", fontWeight: "300"}} >{props.details[i]}</li>
        );
    }
