...
const CityContainer = (props) =>{
  ...
  let isPageWide = media('(min-width: 768px)')
    ...
    let TabsElements = [];
    let TabsContainers = [];
    const _addDayElements =(day_element, day_slab, slab_element_index, day_slab_index) => {
      if(day_slab in DayElements) {}
        ...
        <div>
            <IconElement  is_registration_needed = {props.is_registration_needed}  element_index={day_element.element_index} selectedPoi={props.selectedPoi} activity_data={day_element.activity_data} is_auth={props.is_auth} traveleritinerary={props.traveleritinerary} is_poi_rec={day_element.type === 'POI/Activity Recommendation' ? true : false} is_food={day_element.type === 'Food Recommendation' && isJson(day_element.text)? true : false}  is_preview={props.is_preview} is_stock={props.is_stock} is_experience={props.is_experience} enablepoiedit={day_element.activity_data ? true : false} poi_name={day_element.activity_data ? day_element.activity_data.name : null} day_slab_index={day_slab_index} slab_element_index={slab_element_index} city_id={props.city_id} element_type={day_element.element_type} setShowPoiModal={props.setShowPoiModal}  blur={props.blur} name={day_element.heading} meta={day_element.meta} text={day_element.text} image={day_element.icon}></IconElement>
        </div>
        );
        else
        DayElements[day_slab].push(
          <div>
              <IconElement is_registration_needed = {props.is_registration_needed}  element_index={day_element.element_index}  selectedPoi={props.selectedPoi} activity_data={day_element.activity_data} is_auth={props.is_auth} traveleritinerary={props.traveleritinerary} is_poi_rec={day_element.type === 'POI/Activity Recommendation' ? true : false}   is_food={day_element.type === 'Food Recommendation' && isJson(day_element.text) ? true : false} is_preview={props.is_preview} is_stock={props.is_stock} is_experience={props.is_experience} enablepoiedit={day_element.activity_data ? true : false} poi_name={day_element.activity_data ? day_element.activity_data.name : null}  day_slab_index={day_slab_index} slab_element_index={slab_element_index}  city_id={props.city_id}  element_type={day_element.element_type}   setShowPoiModal={props.setShowPoiModal}  blur={props.blur} meta={day_element.meta} name={day_element.heading} text={day_element.text} image={day_element.icon} type="image"></IconElement>
          </div>
        );
    }
    }
    ...
    for(var j = props.startingslab; j<=props.endingslab; j++){
      if(j === props.startingslab)
        for(var k=props.startingindex; k<props.day_slabs[j].slab_elements.length; k++){
            _addDayElements(props.day_slabs[j].slab_elements[k], props.day_slabs[j].slab, k, j);
    ...
    return(
        <Container className={isPageWide ? 'border' : ' '}>
           {!isPageWide?
            <div style={{display: 'flex', alignItems: 'center', position: 'sticky', left:'0', top: '0vh', backgroundColor: 'white', zIndex: '1000', padding: props.hideTimer ? '12vh 0.5rem 0 0.5rem' : '0.75rem 0.5rem 0 0.5rem', minHeight: '10vh'}}>
