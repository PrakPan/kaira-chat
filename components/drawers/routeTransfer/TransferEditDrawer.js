...
// A stable empty array: the suggestion blocks read `data.trips`/`data.quotes`
// straight off the payload, and a fresh `[]` per render would restart every
// memo hanging off them.
const EMPTY_LIST = [];

/**
 * A search falls back to a convoy only when no single cab seats the party. The
 * result cards then swap their CTA for a quantity stepper and the drawer's own
 * action bar commits the lot as ONE booking — see `useSuggestionFleet`. Only our
 * own fleet can be composed that way; a third-party source quotes whole cars.
 */
 ...
 * addressed by two indices — which route plan, then which cab within it — and
 * both come back loosely typed, so compare them loosely and never let a pair of
 * undefineds match.
 */
const isCommittedQuote = (committed, suggestionIndex, quote) =>
  !!committed &&
  committed.suggestionIndex == suggestionIndex &&
  quote?.result_index != null &&
  committed.quoteIndex == quote.result_index;
...
const TransferEditDrawer = (props) => {
  ...
  // Which quote the last successful commit booked — {suggestionIndex, quoteIndex}.
  // Each result card now commits on its own, so this is what tells the list which
  // one is in the itinerary; a failed commit leaves it untouched on purpose.
  const [committedQuote, setCommittedQuote] = useState(null);
  ...
  // The booked-quote marker addresses a quote by its index in one search's
  // results, so it means nothing against another tab's list.
  useEffect(() => {
    setCommittedQuote(null);
  }, [multicityTab]);
  ...
  // Whether the tab on screen is quoting a convoy, which is the one commit the
  // cards can't make on their own.
  const multicityFleetMode = useMemo(() => {
    const active =
      multicityTab === "multicity"
        ...
        : multicityTab === "sightseeing"
          ...
          : null;
    const list = Array.isArray(active) ? active : active ? [active] : EMPTY_LIST;
    return list.some((sugg) => suggestionNeedsFleet(sugg?.data));
  ...
  const handleMultiCitySelect = (trace_id, result_index, quote_index, vehicles) => {
    ...
    axiosMulticityRoundTripInstance
      ...
      .then((response) => {
        ...
        // Only a fleet has no single quote to point at; every other commit
        // marks its own card as booked.
        setCommittedQuote(
          vehicles?.length
            ? null
            : { suggestionIndex: result_index, quoteIndex: quote_index },
        );
      ...
      >
        ...
        (loadingMulticityTransfers &&
          ...
          <div className="mt-10 w-full flex flex-col gap-3 items-center">
            {["", "", ""].map(() => (
              <>
                <div className="rounded-3xl border-sm border-solid border-[#ececec] p-md relative w-full">
                  <div>
                    ...
                    <div>
                      <div>
                        ...
                        <div className="flex mt-xs">
                          <div className="flex text-[#445069] text-400 ttw-type-body items-center gap-xs w-40">
                            <span>
                              {" "}
                              <SkeletonCard
                                width="20px"
                          ...
                          <div className="flex text-[#445069] text-400 ttw-type-body items-center gap-xs">
                            <span>
                              <SkeletonCard
                                width="20px"
                                height="20px"
          ...
          <div className="w-full flex justify-center py-10 px-4">
            <div className="bg-text-stroke rounded-2xl p-lg flex flex-col items-center text-center max-w-[440px] w-full border-sm border-[#ececec]">
              <div className="w-14 h-14 rounded-full bg-white flex items-center justify-center mb-sm shadow-soft">
                <PiTaxi size={26} className="text-[#445069]" />
              </div>
              <div className="ttw-type-body-strong leading-xl text-[#0b1220] mb-xxs">
                No options available
              </div>
              <p className="ttw-type-small font-400 leading-md text-[#445069]">
                {(() => {
                  const currentTab = multicityTab || "multicity";
                  ...
                  {currentStep === 0 ? (
                    <>
                      {transfers && transfers.length > 0 && (
                        <div
                          className="rounded-3xl border-sm border-solid border-[#ececec] p-md cursor-pointer hover:bg-[#fffde7] relative w-full"
                          key={0}
                          onClick={() => {
                        ...
                        >
                          <div>
                            <div className="bg-[#e7f5ee] text-[#1f8a5a] rounded-full ttw-type-small font-600 leading-lg px-md py-xs inline">
                        ...
                        <div className="w-full">
                          {transfers.slice(1).map((transfer, idx) => {
                            ...
                            return (
                              <div
                                key={index}
                                className="rounded-3xl border-sm border-solid border-[#ececec] p-md cursor-pointer hover:bg-[#fffde7] relative mt-md w-full"
                                onClick={() => {
                  ...
                  multiCitySuggestions.map((sugg, idx) => (
                    <div key={sugg?.result_index ?? idx} className="w-full">
                      <MultiCityTripSuggestion
                        ...
                        traceId={multicityRoundtripTraceId}
                  ...
                  sightseeingDayOptions.length > 0 && (
                    <div className="w-full flex flex-wrap items-end gap-md">
                      <div className="flex flex-col gap-xs">
                        <label className="ttw-type-body font-500 text-[#445069]">
                          Start date
                        </label>
                        <select
                          ...
                          className="border-sm border-solid border-[#ececec] rounded-md-lg px-md py-xs ttw-type-body bg-white"
                        >
                ...
                {multicityTab === "sightseeing" && sightseeingRefetching && (
                  <div className="w-full flex flex-col gap-3 items-center mt-md">
                    {[0, 1, 2].map((i) => (
                      <div
                        key={`ss-skel-${i}`}
                        className="rounded-3xl border-sm border-solid border-[#ececec] p-md w-full"
                      >
                        <SkeletonCard
                          width="40%"
                          height="20px"
                  ...
                  ((multicityTab === "multicity" && !multiCitySuggestions) ||
                    ...
                    <div className="w-full flex justify-center py-10 px-4">
                      <div className="bg-text-stroke rounded-2xl p-lg flex flex-col items-center text-center max-w-[440px] w-full border-sm border-[#ececec]">
                        ...
                        <p className="ttw-type-body leading-md text-[#445069]">
        ...
        {transferType === "MULTICITYROUNDTRIP" &&
          ...
          !transfersError &&
          (() => {
            // Every result card now carries its own CTA, so the only thing left
            // for this bar to commit is a convoy — several cabs picked with the
            // steppers, which is ONE booking and therefore cannot be committed
            // from any single card. Outside that case the bar is dead weight.
            if (!multicityFleetMode) return null;
 ...
 ${
 isDisabled
 ? "bg-[#f7e700] text-black opacity-60 cursor-not-allowed"
 : "bg-[#f7e700] text-black border-1 border-black active:transform active:scale-95 cursor-pointer"
 }
 `}
                    disabled={isDisabled}
                  >
    ...
    <>
      ...
      >
        {transfer[0]?.recommended && (
          <ClippathComp className="ttw-type-body-strong bg-[#F7E700] text-[#0b1220] pl-2 pr-2 py-1 -ml-4 -mt-4 rounded-tl-2xl">
            Recommended
          </ClippathComp>
        )}
        ...
        {selectedResult &&
          ...
          selectedResult.transferIndex === transferIndex && (
            <div className="ttw-type-body text-[#1f8a5a] pb-1">
...
const MultiRoute = (props) => {
  ...
  return (
    <div className="flex flex-row gap-2 w-full">
      <div
        className={`w-[80px] h-[70px] px-2 bg-[#f4f3ec] rounded-xl flex items-center justify-center`}
      >
        <TransfersIcon
...
const toggleTransferDetailsMulti = (priceOptionId) => {
                      ...
                      <div key={key}>
                        ...
                        {isLoading && (
                          <div className="flex justify-center items-center p-4">
                            <PulseLoader size={8} speedMultiplier={0.6} color="#000" />
                            <span className="ml-2 ttw-type-body text-[#445069]">
                        ...
                        {transferErrors[transferKey] && (
                          <div className="p-4 flex flex-col items-center justify-center gap-3 text-center">
                            <span className="ttw-type-small font-400 text-[#445069]">
                              {transferErrors[transferKey]}
                            </span>
                            <OfflineQuoteCTA
                              itinerary_id={itinerary_id}
      ...
      return resultPrices.map(
        (priceOption, priceIndex) => {
          ...
          return (
            <div
              key={priceOptionId}
              className="flex flex-col rounded-3xl border-sm border-solid border-[#ececec] p-md hover:bg-[#fffde7] relative mt-md"
            >
              <div className="flex justify-between max-ph:flex-col">
                <div className="w-full">
                  ...
                  <div className="flex flex-row items-center gap-2">
                    <div className="flex flex-row items-center gap-xs">
                      ...
                      <span className="ttw-type-body-strong text-[#445069]">
                        {operators
                          .map((op) => op.name)
                          .join(" | ")}
                      </span>
                    ...
                    {priceOption.class_name && (
                      <span className="ttw-type-small bg-[#f4f3ec] px-xs py-[2px] rounded-md text-[#445069]">
                        {priceOption.class_name}
                      </span>
                    )}
                  </div>
                  ...
                  {departureInfo && arrivalInfo && (
                    <div className="flex items-center justify-between mt-md mr-2xl max-ph:mr-zero max-ph:mb-md">
                      ...
                      <div className="flex items-center flex-1 mx-md relative">
                        <div className="w-full border-b-[2px] border-black [border-style:dashed] [border-image:repeating-linear-gradient(to_right,#445069_0_6px,transparent_6px_12px)_1]"></div>
                  ...
                  {segments.length >= 1 && (
                    ...
                    >
                      <div className="bg-[#0b1220] text-white rounded-full px-3 py-1 flex items-center gap-2 ttw-type-small md:ttw-type-body font-500">
                        <span className="">
                          Details
                  ...
                  {segments.length >=1 && isExpanded && (
                    <div className="mt-md border-t border-[#ececec] pt-md">
                              ...
                              <div key={segIdx}>
                                ...
                                <div className="flex flex-row items-start gap-3">
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <span
                                      className="ttw-type-body-strong text-[#0b1220] block truncate"
                                      title={
                                        seg
                                          .departure_station
                                          ?.name
                                    ...
                                    <div className="flex items-center gap-2 mt-1 ttw-type-small text-[#445069]">
                                      {seg.operator?.name && (
                                ...
                                {!isLastSegment &&
                                  segments[
                                    ...
                                    <div className="flex flex-row items-center gap-3 mt-3 mb-3">
                                      ...
                                      {/* Transfer badge */}
                                      <div className="flex flex-row items-center gap-2 bg-[#0b1220] text-white px-3 py-1.5 rounded-md">
                                        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="flex flex-col justify-between items-end max-ph:flex-row max-ph:items-center">
                  <div>
                    ...
                    <div className="text-[#445069] ttw-type-body leading-lg">
                      for{" "}
                      {pax?.adults +
                        pax?.children +
                        pax?.infants}{" "}
                                    ...
                                    <div className="flex justify-between max-ph:flex-col">
                                      <div className="w-full">
                                        ...
                                        {priceOption.description && (
                                          <div className="ttw-type-small md:ttw-type-body text-[#445069] mt-1">
                                            {priceOption.description}
                                          </div>
                                        )}
                      ...
                      {comboHasMoreAllAboard && (
                        <div className="flex justify-center mt-md">
                          <button
                            ...
                            className="px-6 py-2 bg-[#0b1220] text-white rounded-lg hover:bg-[#1a2436] transition-colors cursor-pointer ttw-type-body-strong disabled:opacity-50"
                          >
 ...
 ${
 selectedModeIds[currentStep - 1] === option.id
 ? "border border-[#f7e700] bg-[#fffde7]"
 : "border"
 }
 `}
                      >
                        <div className="flex gap-2 md:gap-3 mb-2 md:mb-0">
                          <div className="text-[#8a93a6] mt-1">
                          ...
                          <div>
                            ...
                            <div className="ttw-type-small md:ttw-type-body text-[#445069]">
                        ...
                        <div className="flex flex-row md:flex-col items-end md:items-end justify-between">
                          ...
                          >
                            {selectedModeIds[currentStep - 1] === option.id ? (
                              <div className="flex items-center gap-1">
                                <ImCheckboxChecked className="h-4 w-4 md:h-5 md:w-5 text-[#0b1220]" />
                ...
                <div className="sticky bottom-0 left-0 right-0 w-full bg-[#fafaf5] border-t border-[#ececec] z-[13]">
                  <div className="flex flex-row md:flex-row gap-2 md:gap-0 justify-between items-stretch md:items-center py-3 px-4 md:px-0 max-w-full">
                    {currentStep > 1 ? (
                      <button
                        ...
                        className="border border-[#ececec] bg-white text-[#0b1220] px-4 md:px-6 py-2 rounded-xl font-500 ttw-type-body hover:bg-[#f4f3ec] transition-colors"
                      >
                        Back
                      </button>
                    ) : (
...
const RadioButton = ({ name, label, transferType, handleTransferType }) => {
 ...
 transferType === name ? "bg-primary-yellow" : "bg-[#445069]"
 }`}
          ></div>
        )}
      </div>
...
const useSuggestionFleet = ({ pricing, data, suggestionIndex, onFleetChoice, setSelectedCab }) => {
  ...
  const enabled = suggestionNeedsFleet(data);
...
/**
 * One suggestion — a route plan priced in several vehicle classes.
 *
 * The route is stated once as a header and every priced cab is its own card with
 * its own photo, facts, price and "Add to Itinerary" button, so a customer
 * comparing cabs here is looking at the same list shape the taxi search shows.
 *
 * `RoundTripSuggestion` reads the legacy `transfer_details` quote shape;
 * `MultiCityTripSuggestion` below reads the `taxi_category`/`price` shape mercury
 * sends. Everything else about the two is identical.
 *
 * The exception is a party no single cab seats: `useSuggestionFleet` turns each
 * card's CTA into a quantity stepper and the drawer's own action bar commits the
 * whole convoy as ONE booking — committing per card would only replace the
 * previous car.
 */
const RoundTripSuggestion = ({
  onFleetChoice,
  ...
  const routes = roundTripSuggestions?.data?.trips || EMPTY_LIST;
  const pricing = roundTripSuggestions?.data?.quotes || EMPTY_LIST;

  // The commit names the SUGGESTION here and the quote separately; the legacy
  // payload this component reads has always been suggestion 1.
  const suggestionIndex = roundTripSuggestions?.result_index ?? 1;
  ...
  // The drawer owns the request; once it settles this card's spinner is done,
  // whichever way it went.
  useEffect(() => {
    if (!updating) setPendingQuote(null);
  }, [updating]);

  const handleAdd = (quote) => {
    // A single cab and a fleet are mutually exclusive — the drawer commits
    // whichever is set, so picking one has to clear the other.
    onFleetChoice?.(null);
    setSelectedTripType?.("roundtrip");
    setSelectedCab?.({
      ...quote,
      tripType: "roundtrip",
      suggestion_result_index: suggestionIndex,
    });
    setPendingQuote(quote?.result_index);
    handleRoundTripSelect(
      traceId || roundTripSuggestions?.trace_id,
      suggestionIndex,
      quote?.result_index,
    );
  };
  ...
  return (
    <div className="w-full flex flex-col gap-sm">
      <SuggestionHeader
        title={roundTripSuggestions?.name}
        distance={
          roundTripSuggestions?.data?.distance?.value ??
          roundTripSuggestions?.distance?.value
        }
        duration={
          roundTripSuggestions?.data?.duration?.text ??
          roundTripSuggestions?.duration?.text
        }
        routes={routes}
      />
      ...
      <div className="flex flex-col gap-sm">
        {pricing?.length ? (
          pricing.map((quote, i) => {
            // >1 only when no single cab seats the group; the price shown is
            // then the convoy total, not what one cab costs.
            const vehicleCount = getVehicleCount(quote);
            const total = quote?.transfer_details?.total;
            return (
              <SuggestionQuoteCard
                key={`quote-${quote?.result_index ?? i}`}
                quote={quote}
                currencySymbol={currencySymbol}
                total={total}
                vehicleCount={vehicleCount}
                perVehicleTotal={resolvePerVehicleTotal(
                  quote,
                  total,
                  vehicleCount,
                )}
                isMixedFleet={Boolean(getFleetManifest(quote)?.is_mixed)}
                added={isCommittedQuote(committedQuote, suggestionIndex, quote)}
                ...
                quantity={Number(
                  suggestionFleet.quantities[String(quote?.result_index)] || 0,
                )}
                onQuantityChange={(next) =>
                  suggestionFleet.setQuantity(quote?.result_index, next)
                }
              />
            );
          })
        ) : (
          <div className="ttw-type-body text-[#445069] px-1">
...
const MultiCityTripSuggestion = ({
  ...
  const routes = multiCitySuggestions?.data?.trips || EMPTY_LIST;
  const pricing = multiCitySuggestions?.data?.quotes || EMPTY_LIST;

  const suggestionIndex = multiCitySuggestions?.result_index ?? 0;
  ...
  const handleAdd = (quote) => {
    ...
    setPendingQuote(quote?.result_index);
    handleRoundTripSelect(
      traceId || multiCitySuggestions?.trace_id,
      suggestionIndex,
      quote?.result_index,
    );
  };
  ...
  return (
    <div className="w-full flex flex-col gap-sm">
      <SuggestionHeader
        title={multiCitySuggestions?.name}
        distance={multiCitySuggestions?.data?.distance?.value}
...
const BookedSightseeingCard = ({ booking, onClick }) => {
  ...
  return (
    <div
      ...
      className={`w-full flex flex-row gap-2 items-start rounded-2xl py-3 px-3 pl-2 shadow-sm border-x-2 border-t-2 border-b-4 border-[#1f8a5a] bg-[#e7f5ee] ${onClick ? "cursor-pointer hover:shadow-md transition-shadow" : ""}`}
    >
      {isDesktop && (
...
const BookedAirportCard = ({ booking, isPickup, onViewDetail, onChange }) => {
  ...
  return (
    <div className="w-full flex flex-row gap-2 items-start rounded-2xl py-3 px-3 pl-2 shadow-sm border-x-2 border-t-2 border-b-4 border-[#1f8a5a] bg-[#e7f5ee]">
      {isDesktop && (
        <div className="w-[80px] h-[70px] px-2 bg-white rounded-xl flex items-center justify-center">
          <TransfersIcon
            TransportMode={"Taxi"}
            ...
            <div className="flex flex-row items-center gap-2">
              {onViewDetail && (
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg font-semibold ttw-type-small text-black border-1 border-black bg-white hover:bg-[#f4f3ec] whitespace-nowrap"
                >
                  View Detail
                </button>
              )}
              {onChange && (
                <button
                  type="button"
                  onClick={onChange}
                  className="px-3 py-1.5 rounded-lg font-semibold ttw-type-small bg-[#f7e700] text-black border-1 border-black hover:bg-[#f4f3ec] whitespace-nowrap"
                >
                  Change
                </button>
              )}
...
const AirportPickupDropCard = ({ suggestion, isPickup, onSearch }) => {
  ...
  return (
    <div className="w-full flex flex-row gap-2 items-start rounded-2xl py-3 px-3 pl-2 shadow-sm border-x-2 border-t-2 border-b-4">
      {isDesktop && (
        <div className="w-[80px] h-[70px] px-2 bg-[#f4f3ec] rounded-xl flex items-center justify-center">
          <TransfersIcon
            TransportMode={"Taxi"}
            Instyle={{ fontSize: "3rem", color: "black" }}
            classname={{ width: 80, height: 75 }}
      ...
      <div className="w-full flex flex-col gap-3">
        <div className="flex flex-row gap-2">
          {!isDesktop && (
            <div className="w-[60px] h-[60px] px-2 bg-[#f4f3ec] rounded-xl flex items-center justify-center">
              <TransfersIcon
                TransportMode={"Taxi"}
                Instyle={{ fontSize: "3rem", color: "black" }}
                classname={{ width: 80, height: 75 }}
        ...
        <div className="flex flex-row items-center justify-between gap-2 flex-wrap">
          ...
          <button
            onClick={onSearch}
            className="px-4 py-2 rounded-lg font-semibold ttw-type-body bg-[#f7e700] text-black border-1 border-black hover:bg-[#f4f3ec] active:transform active:scale-95 cursor-pointer transition-all duration-200 ease-in-out"
          >
            Search Taxis
          </button>
        </div>
      ...
      {error && !isCurrentTransferLoading() && (
        <div className="flex justify-center items-center py-8">
          <div className="text-center">
            <div className="text-[#CD2026] ttw-type-h4 mb-2">⚠️</div>
            <div className="ttw-type-body font-600 text-[#0b1220] mb-1">
        ...
        visibleSelfPrices.length > 0 && (
          <div className="w-full flex justify-center px-4 pb-8">
            <div className="w-full max-w-[560px] rounded-3xl border border-[#f7e700] bg-[#fffde7] p-5">
              ...
              <p className="ttw-type-small font-400 leading-relaxed text-[#445069] mb-4">
                  ...
                  return (
                    ...
                    >
                      ...
                      {entryPrices.length === 0 ? (
                        <div className="ttw-type-small font-400 text-[#445069]">
                        ...
                        entryPrices.map((priceOption, priceIdx) => {
                          ...
                          return (
                            <div
                              key={priceOptionId}
                              className="flex items-center justify-between gap-3 rounded-2xl border border-[#ececec] bg-white p-3"
                            >
                              <div className="flex flex-col gap-1">
                                <div className="flex items-center gap-2 flex-wrap">
                                  {priceOption?.class && (
                                    <span className="ttw-type-small bg-[#f4f3ec] text-[#445069] border border-[#ececec] px-2 py-0.5 rounded-full">
                                      {priceOption.class}
                                    </span>
                                  )}
                                  {isEditing ? (
                                    ...
                                    <div className="flex items-center gap-1">
                                      ...
                                      <button
                                        ...
                                        className="text-[#445069] hover:text-[#0b1220] p-1 disabled:opacity-40 disabled:cursor-not-allowed"
                                        title="Edit price"
                                      >
                                        <svg
                                          className="w-4 h-4"
    ...
    return resultPrices.map((priceOption, priceIndex) => {
      ...
      return (
        <div
          key={priceOptionId}
          className={`flex flex-col rounded-3xl border-sm border-solid border-[#ececec] p-md hover:bg-[#fffde7] relative mt-md
 ${isOptionSelected ? "border-[#f7e700] bg-[#fffde7]" : ""}
