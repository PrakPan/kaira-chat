...
const ImageLoaderGoogle = (props) => {
  let isPageWide = media("(min-width: 768px)");
  const isPageLoaded = usePageLoaded();
  const [error, setError] = useState(false);
  const [fullLoaded, setFullLoaded] = useState(false);
  const imgUrlEndPoint = "https://maps.googleapis.com/maps/api/place/photo?photo_reference=";
          ...
          <SmallImage
            src={
              !is_url
                ...
                : `${imgUrlEndPoint}${props.url}&maxwidth=400&key=${GOOGLE_MAPS_API_KEY}`
            }
...
export default ImageLoaderGoogle;
