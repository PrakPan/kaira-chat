...
const ChatHistoryDrawer: React.FC<{
  ...
  return (
    <>
      {open && (
        <div
          className="fixed inset-0 z-[300] bg-black/20 backdrop-blur-[1px]"
          onClick={onClose}
        />
      )}
      <div
...
const SidebarProfile: React.FC<{
  ...
  const floatingDropdown = open && isCollapsed && (
    <div
      className="absolute z-[400] bg-white rounded-xl shadow-xl border border-gray-100 py-1.5 overflow-hidden"
      style={{ bottom: 8, left: "calc(100% + 8px)", minWidth: 200 }}
    >
      {menuItems}
    </div>
  );
