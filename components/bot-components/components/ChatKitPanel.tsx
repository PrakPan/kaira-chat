...
function useUserLocationData() {
  const [userLocationData, setUserLocationData] = useState<UserLocationData | null>(null);
...
const Spinner = ({ size = 16 }: { size?: number }) => (
  <svg width={size} height={size} viewBox="0 0 24 24" fill="none"
    className="animate-spin" style={{ color: "#6b7280" }}>
...
const WelcomeState = () => (
  <div className="flex flex-col items-center justify-center h-full px-6 pb-20 select-none">
    ...
    >
      <span className="text-5xl" role="img" aria-label="travel">🌍</span>
    </div>
...
export function ChatKitPanel({
  ...
  ({ name, data }: { name: string; data: Record<string, unknown> }) => {
    ...
    switch (name) {
      ...
      // ── Widget button actions — send back to backend ──────────────────
      case "route.lock":
      case "route.edit":
      case "route.remove":
  ...
  const apiUrl = botMode === "p2"
    ? "https://chat.tarzanway.com/chatkit/p2"
    : CHATKIT_API_URL;

  const { messages, isStreaming, error, sendMessage, sendWidgetAction, clearMessages, cancelStream } = useChat({
    apiUrl,
