...
// ─── Icons (inline SVGs matching the reference files) ───────────────────────
...
const CheckoutIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="30" height="27" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
...
const getItemImage = (item: SlabElement) =>
  item?.icon ? imgBase + item.icon : fallbackImg;
...
const isActivity = (item: SlabElement) =>
  item?.activity ||
  item?.element_type === "activity" ||
  item?.type === "activity";

const isRecommendation = (item: SlabElement) =>
  item?.element_type === "recommendation" || item?.type === "recommendation";

// ─── ActivityRow (mirrors CityDay.renderActivityItem) ────────────────────────
...
const ActivityRow: React.FC<{ item: SlabElement; index: number }> = ({
  ...
  // Normal activity / POI / restaurant style — with image thumbnail
  return (
    <div className="flex items-center gap-2 md:gap-3 min-w-0 md:flex-1">
      <img
      ...
      <div className="flex flex-col items-start justify-center gap-1 min-w-0 flex-1">
        ...
        <div className="flex items-center gap-2">
          {isActivity(item) && (
            <span className="flex gap-1 items-center px-1.5 md:px-2 py-0.5 bg-[#5CBA66] text-white text-[10px] md:text-xs rounded-full font-medium flex-shrink-0 whitespace-nowrap">
              <IncludedIcon />
              Included
            </span>
          )}
...
const DayRow: React.FC<DayRowProps> = ({ day, isLastDay, cityName }) => {
  // Prioritise: activities → pois → restaurants → recommendations (matching CityDay)
  ...
  for (const elem of day.slab_elements || []) {
    if (elem?.activity || elem?.element_type === "activity" || elem?.type === "activity") {
      activities.push(elem);
    } else if (elem?.poi) {
      pois.push(elem);
    } else if (elem?.restaurant) {
      restaurants.push(elem);
    } else if (elem?.element_type === "recommendation" || elem?.type === "recommendation") {
...
// ─── HotelRow (mirrors ItineraryCity hotel display) ──────────────────────────
...
const HotelRow: React.FC<HotelRowProps> = ({ hotels }) => {
  if (!hotels) {
    return (
      <button className="text-blue-600 cursor-pointer text-[14px] font-medium hover:underline">
        + Add Stay
      </button>
    );
  }

  // If hotels is a plain string (legacy HotelInfo prop)
  ...
  return (
    <div className="flex flex-col gap-1">
      {hotels.map((hotel) => (
        <div key={hotel.id} className="flex flex-row items-center mt-3">
          <div className="flex gap-2 pr-[8px]">
            <HotelIcon />
            <span className="text-[14px] font-400 cursor-pointer hover:underline">
              {hotel.name}
            </span>
          </div>
          {hotel.rating && hotel.rating !== 0 ? (
            <div className="flex items-center pl-[8px]">
              <span className="text-[#000] text-[12px] ml-1 mr-1 font-[500]">
                {hotel.rating}
              </span>
              <StarIcon />
            </div>
...
const CitySection: React.FC<CitySectionProps> = ({ route }) => {
  return (
    <div className="rounded-t-lg flex flex-col w-full bg-[#FFF5EF]">
      {/* ── City header — matches ItineraryCity top bar ── */}
      <div className="flex items-start justify-between p-3 rounded-t-lg w-full border border-[#FBEAC7]">
        <div className="space-y-1 font-inter w-full">
          {/* City name + duration */}
          <div className="md:text-[18px] font-semibold leading-none">
            {route.city.name}
            {" - "}
            {route.city.duration}{" "}
      ...
      {/* ── Day-by-day rows — matches CityDaybyDay / CityDay ── */}
      <div className="border border-t-0 border-[#E8E8E8] rounded-b-lg overflow-hidden bg-white">
