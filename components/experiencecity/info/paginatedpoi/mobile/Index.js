...
const Inlcusions = (props) => {
    ...
    useEffect(() => {
          ...
          axiosthingsinstance
          .get(
            `/?slug=`+props.slug+'&limit=6&offset=0'
          )
          .then((res) => {
              let newArr = [];
            let newJSXArr = [];
    ...
    const _showMoreHandlerNew = () => {
        ...
        axiosthingsinstance
        .get(
          `/?slug=`+props.slug+'&limit=6&offset='+newoffset
        )
        .then((res) => {
            let newJSXArr = [];
         ...
         if(!res.data.results.length) setShowMoreStatus(false);
        else{
          for(var i = 0; i<res.data.results.length; i++){
              newArr.push({...res.data.results[i]});
              newJSXArr.push(
    ...
    <div>
        ...
        {showMoreStatus ? <Button  boxShadow  margin="1rem auto" borderRadius="2rem" borderWidth="1px" padding="2px 1.25rem" onclick={_showMoreHandlerNew}>
            Show More
            </Button> 
...
{/* <Button    margin="1rem auto" borderRadius="2rem" borderWidth="1px" padding="2px 1.25rem" onclick={_showMoreHandlerNew}>
            Show More
            </Button>  */}