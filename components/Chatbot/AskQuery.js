...
const AutoGrowTextarea = styled.textarea`
  ...
  &::placeholder {
    color: transparent; /* hidden; we use overlay instead */
  }
`;
...
const PlaceholderOverlay = styled.span`
  ...
  overflow: hidden;
  max-width: 100%;
  // animation: ${(p) => (p.$out ? slideUpOut : slideUpIn)} 0.35s ease forwards;
`;
...
const SingleChip = styled.button`
  border-radius: 6px;
  padding: 8px 12px;
...
function AskQuery() {
  ...
  // Optional: set these to rotate placeholders like MessageInputBox supports
  const rotatePlaceholders = ["Ask me anything", "Where do you want to go?", "What can I help you plan?"];
  ...
  const canSend = query.trim().length > 0 && !disableQuerySection;

  const handleSubmitQuery = (input) => {
    const msg = input || query.trim();
    if (!msg) return;
    const userMsg = { is_bot: false, message: msg };
    sendMessage(userMsg);
  ...
  return (
    <GrowTextAreaBox>
      <Container>
        ...
        {quickReplies.length > 0 && (
          <ChipsContainer>
            {quickReplies.map((item, index) => (
              <SingleChip key={index} onClick={() => handleSubmitQuery(item)}>
                {item}
              </SingleChip>
            ))}
          </ChipsContainer>
        ...
        {/* Input card — matches MessageInputBox exactly */}
        <InputCard>
          ...
          <div style={{ position: "relative" }}>
            ...
            {!query && (
              <PlaceholderOverlay key={0} 
              // $out={fadingOut}
              >
                {rotatePlaceholders[0]}
              </PlaceholderOverlay>
            )}
          </div>
          ...
          <ActionRow>
            {/* Left: attach */}
            <IconBtn type="button" title="Attach">
              <PlusIcon />
            </IconBtn>

            {/* Right: voice + send */}
            <RightActions>
              <IconBtn type="button" title="Voice input">
                <Dictate
                  ref={dictateRef}
                  stopDictation={stopDictation}
                  onTranscriptChange={handleTranscriptChange}
                  customTrigger={<WaveformIcon />}
                />
              </IconBtn>
