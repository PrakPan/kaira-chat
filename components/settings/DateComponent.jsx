      ...
      <div>
        <div>
          ...
          <div className="relative w-full">
            <StyledFigmaBox
              value={
                props.date.type === "fixed"
                  ? props.date.start_date && props.date.end_date
                    ? `${getHumanDate(
                        props.date.start_date instanceof Date
                          ? props.date.start_date
                              .toLocaleDateString("en-CA")
                              .split("-")
                              .reverse()
                              .join("/")
                          : props.date.start_date.split("-").reverse().join("/")
                      )} - ${getHumanDate(
                        props.date.end_date instanceof Date
                          ? props.date.end_date
                              .toLocaleDateString("en-CA")
                              .split("-")
                              .reverse()
                              .join("/")
                          : props.date.end_date.split("-").reverse().join("/")
                      )}`
                    : ""
                  : props.date.type === "flexible"
                  ? new Date(props.date.month).toLocaleString("default", {
                      month: "long",
                    }) +
                    ", " +
                    props.date.duration +
                    " days"
                  : props.date.duration + " days"
              }
