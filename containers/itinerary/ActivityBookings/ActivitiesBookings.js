        ...
        return city?.activities?.map((item, index) => {
          ...
          >
            <div className="font-bold lg:text-2xl text-xl pb-2 text-[#01202B]">
              {city?.name} {item.duration && <span>({item?.duration})</span>}
            </div>
            <div
              id={index}
              ...
              <div className="relative flex lg:flex-row w-full flex-col gap-4  grayscale-0 ">
                <div className="relative lg:h-[15rem] lg:w-[30%] w-full h-[12rem]">
                  <ImageLoader
                    ...
                    leftalign
                    widthmobile="100%"
                    url={item?.image ? item.image : item.activity?.image}
                  ></ImageLoader>
                ...
                <div className="flex flex-col gap-4 text-[#01202B] lg:w-[70%]">
                  ...
                  <div className="flex flex-col gap-2 -mt-2">
                    ...
                    {item?.activity?.rating ? (
                      <div className="gap-1 flex flex-row  items-center">
                        <div className="flex flex-row text-[#FFD201]">
                          {starRating(item?.activity?.rating)}
                        </div>
                        <div>{item?.activity?.rating}</div>
                        {item?.activity?.user_ratings_total && (
                          <div className="text-sm text-[#7A7A7A] font-medium underline">
                            {item?.activity?.user_ratings_total} Reviews
                          </div>
                        )}
                      </div>
                    ) : null}
                    ...
                    {item?.points && item?.points.length ? (
                      ...
                      <div className="font-light">
                        {item?.activity?.short_description.slice(0, 250)}
                        {item?.activity?.short_description.length ? (
                          <button
                            id={index}
                            onClick={handleMoreDiscription}
                      ...
                      <div className="flex flex-row gap-2 items-center">
                        ...
                        <div>
                          <div className="text-sm font-[400] line-clamp-1">
                            {item.check_in && getDate(item.check_in)}
                            {item.check_out &&
                              " - " + " " + getDate(item.check_out)}
                          </div>
                        </div>
                      </div>
                    )}
                    {item.activity?.ideal_duration_hours_text && (
                  ...
                  <POIDetailsDrawer
                    ...
                    image={item?.image}
                    icon={item?.icon}
                    text={item?.activity?.short_description}
                    Topheading={"Select Our Point Of Interest"}
                    activityData={{ ...item, type: "activity" }}
