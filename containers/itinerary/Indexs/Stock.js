...
const Itinerary = (props) =>{
  const _reloadBookingsHandler=()=> {
    ...
    useEffect(() => {
        ...
        window.scrollTo(0,0);
        axios.get(MIS_SERVER_HOST+"/sales/itinerary/stock/day_by_day/?itinerary_id="+props.id)
          .then(res => {
          ...
          axios.get(MIS_SERVER_HOST+"/sales/itinerary/stock/brief/?itinerary_id="+props.id)
          .then(res => {
            setBreif(res.data);
...
for(var i=0; i<breif.city_slabs.length; i++){
  if(breif.city_slabs[i].duration) totalduration+=parseInt(breif.city_slabs[i].duration);
}
  if(!loading){
    if(!booking){
  ...
  if(!payment){
    fetch(MIS_SERVER_HOST+"/payment/info/?itinerary_type=Tailored&itinerary_id="+itinerary.tailor_made_id)
    .then(response =>  {
        if(response.status === 200){
          response.json().then(json => {
            // setPayment(json);
        ...
        <Container>
          {/* <Header/> */}
           <FullImg   url={itinerary.images ? itinerary.images[0] : null}>
                 <FullImgContainer heading={itinerary.name}  duration={totalduration}></FullImgContainer>
           </FullImg> 
            <div id="itinerary-anchor">
              <Menu _reloadBookingsHandler={_reloadBookingsHandler} id={props.id} is_stock _updateBookingHandler={_updateBookingHandler} showbooking={showbooking}  payment={payment} itinerary={itinerary} breif={breif} booking={booking}></Menu>
            </div>
            {/* <Footer></Footer> */}
        </Container>
    );
