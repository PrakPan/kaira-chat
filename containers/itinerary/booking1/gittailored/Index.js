      ...
      for(const booking in props.payment.costings_breakdown){
        ...
        if(props.payment.costings_breakdown[booking].user_selected){
          if(props.payment.costings_breakdown[booking].booking_type==="Accommodation"){
              bookingslist.push(
                <p style={{fontSize: "0.75rem", fontWeight: "400", letterSpacing: "1px", marginBottom: '0.25rem'}} className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{props.payment.costings_breakdown[booking].detail["duration"]+"N at "+ props.payment.costings_breakdown[booking].detail["name"]}</p>
              )
              bookinglistwithcost.push(
                <div style={{display: 'grid', gridTemplateColumns: '3fr 1fr', margin: '0.5rem 0', gridGap: '1rem'}}>
                  <p style={{ fontSize: "0.75rem", fontWeight: "300", letterSpacing: "1px", marginBottom: '0.25rem'}} className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{props.payment.costings_breakdown[booking].detail["duration"]+"N at "+ props.payment.costings_breakdown[booking].detail["name"]}</p>
                  <p style={{fontSize: "0.75rem", fontWeight: "300", letterSpacing: "1px", marginBottom: '0.25rem'}}  className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{"₹ " + getIndianPrice(Math.ceil(props.payment.costings_breakdown[booking]["booking_cost"]/100)) }</p> 
              </div>
            )
          }
            ...
            bookingslist.push(
              <p style={{fontSize: "0.75rem", fontWeight: "400", letterSpacing: "1px", marginBottom: '0.25rem'}}  className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{ props.payment.costings_breakdown[booking].detail["name"]}</p>
              )
              bookinglistwithcost.push(
                <div style={{display: 'grid', gridTemplateColumns: '3fr 1fr', margin: '0.5rem 0', gridGap: '1rem'}}>
                          <p style={{fontSize: "0.75rem", fontWeight: "300", letterSpacing: "1px", marginBottom: '0.25rem'}}  className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{ props.payment.costings_breakdown[booking].detail["name"]}</p>
                          <p style={{fontSize: "0.75rem", fontWeight: "300", letterSpacing: "1px", marginBottom: '0.25rem'}}  className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{"₹ " + getIndianPrice(Math.ceil(props.payment.costings_breakdown[booking]["booking_cost"]/100)) }</p>
                        {/* <div></div> */}
                </div>
              )
...
const _startRazorpayHandler = (data) => {
    ...
    {/* <Heading bold blur={props.blur} margin="0 auto 1.5rem auto" noline align="center">{ props.hasUserPaid ? "You're all set!" : 'Book Now'}</Heading> */}
        ...
        {!oldaccommodation ? <div style={{marginBottom: '1.5rem', display: "grid", gridTemplateColumns: "1fr 1fr", gridColumnGap: "1rem"}}>
                {props.payment.itinerary_status === ITINERARY_STATUSES.itinerary_finalized || props.plan.featured  ? <p style={{fontSize: "0.75rem", fontWeight: "600", letterSpacing: "1px", marginBottom: '0.25rem'}} className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{"STARTING DATE "}</p> : <div></div>}
                {props.payment.itinerary_status === ITINERARY_STATUSES.itinerary_finalized || props.plan.featured ? <p style={{fontSize: "0.75rem", fontWeight: "600", letterSpacing: "1px", marginBottom: '0.25rem'}}  className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>PAX</p>  : <div></div>}
                {props.payment.itinerary_status === ITINERARY_STATUSES.itinerary_finalized || props.plan.featured ? <div style={{display: 'flex', alignItems: 'center', fontSize: "0.75rem", fontWeight: "400", letterSpacing: "1px", marginBottom: '0'}}  className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{props.payment.meta_info ? props.payment.meta_info.start_date ?  getHumanDate(props.payment.meta_info.start_date.replaceAll('-','/')) :  null :null}</div> : <SelectDate date={date} setDate={setDate} token={props.token}></SelectDate>}
                ...
                {props.payment.meta_info && (props.payment.itinerary_status === ITINERARY_STATUSES.itinerary_finalized || props.plan.featured )? <div>
                  ...
                  <p className='font-lexend' style={{marginRight: '1rem', display: 'inline', fontWeight: '100'}}>{props.payment.meta_info.number_of_adults}</p>
                  <FontAwesomeIcon icon={faChild} style={{marginRight: '0.25rem'}}></FontAwesomeIcon>
                  <p className='font-lexend' style={{marginRight: '1rem', display: 'inline', fontWeight: '100'}}>{props.payment.meta_info.number_of_children}</p>
                  <FontAwesomeIcon icon={faBaby} style={{marginRight: '0.25rem'}}></FontAwesomeIcon>
                  <p className='font-lexend' style={{marginRight: '1rem', display: 'inline', fontWeight: '100'}}>{props.payment.meta_info.number_of_infants}</p>
...
4. WrappedApp created new store with withRedux(MyApp) { initialState: undefined,
  initialStateFromGSPorGSSR: undefined }
           ...
           {!oldaccommodation && !props.payment.are_prices_hidden ? <div style={{display: 'grid', gridTemplateColumns: 'auto max-content', margin: '0.5rem 0', gridGap: '1rem'}}>
                  <p style={{fontSize: "0.75rem", fontWeight: "300", letterSpacing: "1px", marginBottom: '0.25rem'}} className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{'Service Fee'}</p>
                  <p style={{fontSize: "0.75rem", fontWeight: "300", textAlign: 'right', letterSpacing: "1px", marginBottom: '0.25rem', marginRight: '24px'}}  className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{"₹ "+ getIndianPrice(Math.round(props.payment.total_service_fee/100))}</p>
          </div> : null}
          {!oldaccommodation && !props.payment.are_prices_hidden  ? <div style={{display: 'grid', gridTemplateColumns: 'auto max-content', margin: '0.5rem 0', gridGap: '1rem'}}>
                  ...
                  <p style={{fontSize: "0.75rem", textAlign: 'right', fontWeight: "300", letterSpacing: "1px", marginBottom: '0.25rem', marginRight: '24px'}}  className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{"₹ "+ getIndianPrice(Math.round(props.payment.gst/100))}</p>
          </div> : null}
          {/* <div style={{display: 'grid', gridTemplateColumns: '3fr 1fr', margin: '0.5rem 0', gridGap: '1rem'}}>
                  <p style={{fontSize: "0.75rem", fontWeight: "300", letterSpacing: "1px", marginBottom: '0.25rem'}} className={props.blur ? "font-lexend text-enter blurry-text" : "font-lexend text-enter"}>{'GST'}</p>
     ...
     <StrikedCost show_per_person_cost={props.payment.show_per_person_cost} coupon={props.payment.coupon}  className={props.blur ? "font-lexend blurry-text" : "font-lexend"}><FontAwesomeIcon style={{marginRight: '2px'}} icon={faRupeeSign} ></FontAwesomeIcon>{getIndianPrice(Math.round(props.payment.per_person_total_cost/100)*2) }</StrikedCost>
     <INR show_per_person_cost={props.payment.show_per_person_cost} coupon={props.payment.coupon}  className={props.blur ? "font-lexend blurry-text" : "font-lexend"}><FontAwesomeIcon style={{marginRight: '0.25rem'}} icon={faRupeeSign}/>{ getIndianPrice(Math.round(Math.round(props.payment.per_person_total_cost)/100))+ " /-" }</INR>
     </div>
     ...
     {/* <Button blur={props.blur} width="100%" bgColor="#F7e700" borderRadius="5px" borderWidth="0px" margin="0 0 0.5rem 0" onclick={_startCheckoutHandler} ><p style={{margin: '0'}} className={props.blur ? "blurry-text" : ''}>Proceed</p></Button> */}
       ...
       Proceed to Payment</Button> */}
        ...
        Login</Button>}
        {
          ...
          props.payment? props.payment.itinerary_status === ITINERARY_STATUSES.itinerary_finalized ?
           <p onClick={() => setShowRegisteredUsers(true)} className='hover-pointer font-lexend text-center' style={{fontSize: '0.85rem', textDecoration: 'underline'}}>Registered Members</p>
           : null : null}
         {
          props.payment && props.token && ( props.payment.itinerary_status === ITINERARY_STATUSES.itinerary_finalized ) && !props.payment.paid_user ? props.payment.email_reverification_needed  ? <Button borderRadius="5px" bgColor="#f7e700" width="100%" margin="0 0 0.25rem 0" hoverBgColor="black" hoverColor="white" borderWidth="0"   onclick={ setShowVerification } onclickparam={true} >
      ...
      <FontAwesomeIcon icon={faWhatsapp} style={{marginRight: "0.5rem"}}/>
       Connect on WhatsApp</Button>
       <div style={{color: 'blue', margin: '1rem 0 0 0', fontSize: '0.85rem'}} className=" text-center hover-pointer font-lexend" onClick={() => setShowTerms(true)}>Terms & Conditions</div>
