  ...
  let bookingstest = [
    {
      ...
      stay_duration: 3,
      amenities: ['Conference Room', 'Swimming Pool', 'Daily House Keeping'],
  ...
  useEffect(() => {
          ...
          if (props.booking[i].booking_type === 'Accommodation')
            bookingcities[props.booking[i].city].push(
              <OldBookingCard
                ...
                city={props.booking[i].city}
                type={props.booking[i].booking_type}
                key={i}
                ...
                accommodation
                heading={props.booking[i]['name']}
                setImagesHandler={_setImagesHandler}
                rating={props.booking[i]['user_rating']}
                details={props.booking[i]['points']}
                rating={props.booking[i]['weighted_rating']}
                images={props.booking[i]['images']}
                price={props.booking[i]['booking_cost']}
                number_of_rooms={props.booking[i]['number_of_rooms']}
                check_in={props.booking[i]['check_in']}
                check_out={props.booking[i]['check_out']}
                room_type={props.booking[i]['room_type']}
              ></OldBookingCard>
            );
          else
            bookingcities[props.booking[i].city].push(
        ...
        // bookingcities[props.booking[i].city].push(props.booking[i])
          ...
          if (props.booking[i].booking_type === 'Accommodation') {
            ...
            if (props.booking[i].costings_breakdown.length)
              number_of_rooms =
                props.booking[i].costings_breakdown[0]['number_of_rooms'];
            if (
              !props.booking[i].user_selected &&
              !props.booking[i].alternate_to
            ) {
              null;
              ...
              alternatesarr[props.booking[i].alternate_to].push(
                props.booking[i]
              );
            } else if (props.booking[i].user_selected)
              bookingcities[props.booking[i].city].push(
                <BookingCard
                  star_category={props.booking[i].star_category}
                  duration={props.booking[i].duration}
                  city={props.booking[i].city}
                  is_auth={props.is_auth}
                  tag={props.booking[i].tag}
                  are_prices_hidden={props.payment.are_prices_hidden}
                  rooms={props.booking[i].costings_breakdown}
                  ...
                  price={props.booking[i]['booking_cost']}
                  price_type={
                    props.booking[i].costings_breakdown.length
                      ? props.booking[i].costings_breakdown[0]['pricing_type']
                      : null
                  }
                  number_of_rooms={props.booking[i]['number_of_rooms']}
                  check_in={props.booking[i]['check_in']}
    ...
    for (var key in bookingcities) {
      if (bookingcities[key].length) {
        bookingsDesktop.push(
          <Heading blur={props.blur} bold margin="0 0 1rem 0">
            {key}
          </Heading>
        );
        ...
        bookingsFlickity.push(<Flickity cards={bookingcities[key]}></Flickity>);
        bookingsDesktop.push(
          <CardsContainer>{bookingcities[key]}</CardsContainer>
        );
      }
    }
            ...
            <Container>
              <BookingsContainer style={{ marginTop: '0' }}>
                <MessageContainer className="border-thin font-lexend">
                  Here are a few recommendations for booking your travel
                  experience that you can completely edit on your own. Our
                  experience captain will get in touch with you to help you out.
                  🙂
                </MessageContainer>
                {bookingDesktopJSX}
              ...
              {props.showBookingModal ? (
                <BookingModal
                  ...
                  tailored_id={props.booking[0]['tailored_itinerary']}
          ...
          <Container style={{ marginTop: '0' }}>
            ...
            {!showpayment ? (
              ...
              >
                ...
                <Button
                  width="100%"
                  bgColor={props.traveleritinerary ? 'white' : '#F7e700'}
                  borderRadius="5px"
                  borderWidth={props.traveleritinerary ? '1px' : '0px'}
                  margin="0.5rem 0 0.5rem 0"
                  borderColor="#e4e4e4"
                  onclick={_showPaymentHandler}
                >
                  <p
                    style={{ margin: '0' }}
                    className={props.blur ? 'blurry-text' : ''}
                  >
                    {props.traveleritinerary ? 'View Details' : 'Buy Now'}
                  </p>
                </Button>
                <Button
                  onclick={() =>
