...
const Details = (props) => {
  ...
  const _getBasePrice = (payment_info, starting_point) => {
    for (var i = 0; i < payment_info.length; i++) {
      if (payment_info[i]["starting_point"] === starting_point)
        return payment_info[i].base_price;
    }
    return null;
  };
  const _calculateServiceFee = (starting_point, pax) => {
    if (props.payment["service_fee_type"] === "% of base price") {
      if (props.payment["service_fee_multiplier"] === "Multiply") {
        ...
        const baseprice = _getBasePrice(
          props.payment.payment_info,
          starting_point
        );
        // calculate final amount using base price, pax, service fee %
        const totalprice =
          baseprice * pax +
          (props.payment["service_fee_value"] / 100) * baseprice * pax;
  ...
  return (
    <SummaryContainer className="border-thin">
      ...
      <Heading bold margin="0 auto 1.5rem auto" align="center">
        Book Now
      </Heading>
      <div
        style={{
