    ...
    <div className="bg-white px-2 rounded-lg">
      ...
      <div className="mb-2 flex justify-between max-ph:flex-col">
        ...
        {/* The traveller block rides at the top of this column — this row is
            ...
            button below it. */}
        <div className="flex flex-col items-end gap-sm shrink-0 max-ph:items-start">
          {travellerSummary}
          <GetInTouchContainer>
            <Button
              color="#fff"
          ...
          <div className="w-full py-md border-y-sm text-left flex items-center justify-between">
            <div className="flex items-center w-full justify-between">
              <div className="flex items-center gap-2">
                <span className="text-sm font-400 leading-lg">
                  Apply coupon
                </span>
              </div>
          ...
          >
            ...
            >
              ...
              {categoryTotal > 0 && (
                <>
                  {!arePricesHidden && (
                    <div className="text-sm-md leading-xl font-500 border-r-sm border-text-disabled pr-md mr-sm">
                      {currencySymbols?.[currency]
                        ? currencySymbols?.[currency]
                        : "₹"}{" "}
...
const Details = (props) => {
  ...
  const travellerCountLabel = () => {
    const children = props.itinerary?.number_of_children || 0;
    const infants = props.itinerary?.number_of_infants || 0;
    const parts = [`${pax} ${pluralDetector("Adult", pax)}`];
    if (children)
  ...
  const renderTravellerSummary = ({ leadName } = {}) => (
    <div className="flex flex-col items-end gap-xxs shrink-0 mt-1">
      {leadName ? (
        <span className="text-sm-md font-400 leading-lg">{leadName}</span>
      ) : null}
      <span className="text-xs font-400 leading-md text-text-spacegrey text-right">
        Travellers: {travellerCountLabel()}
      </span>
      <span
