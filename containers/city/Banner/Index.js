...
const  Banner = (props) => {
    ...
    const _handleScroll = ( ) => {
        ...
        if(window.pageYOffset > 350){
                if(props.data.short_description){
                    if(props.offsets['Overview'] - window.pageYOffset <= 70 && props.offsets['Overview'] - window.pageYOffset > 0) {
                        if(heading !== 'Overview') setHeading('Overview')
                    }
                }
                if(props.data.pois.length){
                    if(props.offsets['Things to do'] - window.pageYOffset <= 70 && props.offsets['Things to do'] - window.pageYOffset > 0) {
                        if(heading !== 'Things to do') setHeading('Things to do')
                    }
                }
                if(props.data.survival_tips_and_tricks){
                     if(props.offsets['Survival Tips & Tricks'] -  window.pageYOffset <= 70 && props.offsets['Survival Tips & Tricks'] -  window.pageYOffset > 0){
                    // if(heading !== 'Route') setHeading('Route')
                            if(heading !== 'Survival Tips & Tricks') setHeading('Survival Tips & Tricks')
                    }
                 }
                if(props.data.folklore_or_story){
                    if(props.offsets['Folklore or Story'] -  window.pageYOffset <= 70 && props.offsets['Folklore or Story'] -  window.pageYOffset > 0){
                        if(heading !== 'Folklore or Story') setHeading('Folklore or Story')
                    }
                }
                if(props.data.foods.length){
                    if(props.offsets['What to Eat'] -  window.pageYOffset <= 70 && props.offsets['What to Eat'] -  window.pageYOffset > 0){
                        if(heading !== 'What to Eat') setHeading('What to Eat')
                    }
                }
                if(props.data.conveyance_available){
                    if(props.offsets['Getting Around'] -  window.pageYOffset <= 70 && props.offsets['Getting Around'] -  window.pageYOffset >  0){
                ...
                if(props.data.experiences.length){
                    if(props.offsets['Experiences'] -  window.pageYOffset <= 70 && props.offsets['Experiences'] -  window.pageYOffset >  0){
  ...
  const handleClose = () => {
   ...
   return( 
    <FixedContainer style={{display : props.experienceLoaded ? 'initial' : 'none'}}>
        {anchorEl ? <Menu data={props.data} locations={props.locations} hideMenu={handleClose} /> : null}
    <Container className="border" >
    <div style={{display: 'flex'}}>
        {/* <FontAwesomeIcon icon={faSortUp} style={{margin: "0"}}/> */}
        <MenuButton   handleClick={handleMenuClick}></MenuButton>
        <NavigationHeading className="font-opensans" onClick={() => handleMenuClick()}>{heading}</NavigationHeading>
    </div>    
    <div>
     ...
     <div>
            ...
            <Button boxShadow  borderRadius="5px" borderWidth="0px" bgColor="#F7e700" color="black" padding="0.5rem 1rem" margin="0.5rem 0" onclick={_handlePlanning}>Start Planning</Button>
    </div>
    </Container>
    </FixedContainer>
  );
