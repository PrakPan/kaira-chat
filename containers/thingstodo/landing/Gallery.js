...
const Gallery = (props) => {
      let ImageContainerTopPadding = localStorage.getItem('NavbarHeight');
      let isPageWide = media('(min-width: 768px)')

 const imageClickHandler = () => {
    props.setGalleryOpen(true)
  }
  const imageheight = Math.round(window.innerHeight / 2);
  const imagewidth = Math.round(window.innerWidth / 4)
  return(
