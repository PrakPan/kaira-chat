...
const SignIn = (props) => {
  ...
  for(const country in extensions){
    ...
    const _submitHandler = () => {
      ga.event({action: 'Experience-enquiry-initiate', params: {key : ''}})
                ...
                "locations": props.experience,
                  ...
                  axiostailoredinstance.post("", data, {headers: {
                    'Authorization': `Bearer ${props.token}`
                      ...
                      ga.event({action: 'Experience-enquiry-success', params: {key : ''}})
                      ...
                      setLoading(false);
                      ga.event({action: 'Experience-enquiry-fail', params: {key : ''}})
...
useEffect(() => {
  if(props.token && props.name){
   setName((props.name));
  }
}, [props.name, props.token, props.phone]);
...
if(props.token && props.phone && props.phone !== 'null')
  return (
    <Container component="main" className={classes.box} maxWidth="xs">
      {!isPageWide && props._showBookingMobileHandler ?<FontAwesomeIcon icon={faTimes} onClick={props._showBookingMobileHandler} style={{float: 'right', marginBottom: '0.5rem'}}></FontAwesomeIcon> : null}
        <Heading bold margin="0 auto 1.5rem auto" noline align="center">{props.name ? "Hi "+getFirstName(props.name)+" !" : "Hi Travaler!"}</Heading>
      ...
      <Grid container spacing={2}>
           <Grid item xs={12}>
              <TextField
                ...
                type="message"
                id="message"
                onChange={(event) => _userDetailsOnChangeHandler(event, 'message')}
              />
            </Grid>
        <Grid item xs={12}>
                {/* <Button  hoverBgColor="black"  onClick={() => _submitHandler()} width="100%" bgColor="#F7e700" hoverColor="black" borderRadius="5px" borderWidth="0px" margin="0" hoverBgColor="#f7e700">
                    Send Enquiry{loading ? <Spinner display="inline" size={16} margin="0 0 0 0.5rem"></Spinner> : null}
                </Button> */}
                 <Button  hoverBgColor="black"  onClick={() => _submitHandler()} width="100%" bgColor="#F7e700" hoverColor="black" borderRadius="5px" borderWidth="0px" margin="0" >
                    Send Enquiry{loading ? <Spinner display="inline" size={16} margin="0 0 0 0.5rem"></Spinner> : null}
                </Button>
  ...
  <LoginContainer className="border-thin" style={{width: props.bookings ? '100%' : '35%'}}>
      <Login noclose></Login>
  ...
  if(props.token && props.phone && props.phone !== 'null'){
    return (<Container component="main" className={classes.box} maxWidth="xs" style={{margin: '1rem 0', padding: '1rem'}}>
    {!isPageWide && props._showBookingMobileHandler ?<FontAwesomeIcon icon={faTimes} onClick={props._showBookingMobileHandler} style={{float: 'right', marginBottom: '0.5rem'}}></FontAwesomeIcon> : null}
      <Heading bold margin="0 auto 1.5rem auto" noline align="center">{"Hi "+name+" !"}</Heading>
      <Subheading className="font-nunito">How can we help you?</Subheading>
    <Grid container spacing={2}>
         <Grid item xs={12}>
            <TextField
      ...
      <SharedButton boxShadow onclick={()=> window.location.href="https://wa.me/919625509382?text="+message} hoverColor="black" hoverBgColor="#128C7E"  onclickparam={null} width="100%" bgColor="white" borderRadius="5px" borderWidth="1px" borderColor="#e4e4e4"   margin="0" >     
    ...
    <LoginContainer className="border-thin" style={{margin: '1rem 0', padding: '1rem'}}>
        <Login noclose></Login>
