...
return(
    <div>
        <Modal  backdrop={true} show={props.show}  size="md" centered onHide={_hideModalHandler} style={{padding: "0"}}>
            ...
            <Heading>{submitted ? "Thank you for reaching out" : "Let's Connect" }</Heading>
            <div>
            </div>
            {!submitted ? 
            <Grid container spacing={2}>
                <Grid item xs={6}>
                    <TextField onFocus={() => setPersonError(false)} error={personError ? true : false } helperText={personError ? personError : null} label="Your Name" placeholder="Enter your name" key="name"  variant="outlined" required fullWidth name="name" label="Your Name" type="name" id="name"   onChange={(event) => setName(event.target.value) } onBlur={null}/>
                </Grid>
                <Grid item xs={6}>
                    <TextField onFocus={() => setCompanyError(false)} error={companyError ? true : false } helperText={companyError? companyError : null}  placeholder="Enter company name" key="company_name"  variant="outlined" required fullWidth name="company_name" label="Company Name" type="name" id="company_name" onChange={(event) => setCompanyName(event.target.value)} onBlur={null}/>
                </Grid>
                <Grid item xs={6}>
                ...
                <Grid item xs={6}>
                    <TextField onFocus={() => setEmailError(false)} error={emailError ? true : false } helperText={emailError ? emailError : null}  placeholder='info@thetarzanway.com' key="email"   variant="outlined" required fullWidth name="email" label="Email ID" type="email" id="email" onChange={(event) => setEmail(event.target.value)} onBlur={null}/>
                </Grid>
                <Grid item xs={12}>
  ...
  onClose={() => setCalendarOpen(false)}
      ...
      />
                ...
                <Grid item xs={12}>
                    {!loading ? 
                    <Button onclickparam={null} onclick={_submitDataHandler} margin="1rem 0 0 0"  width="100%" borderRadius="5px" borderWidth="0" bgColor="#f7e700" hoverBgColor="black" color="black" hoverColor="white">Schedule Callback</Button>
                        : 
