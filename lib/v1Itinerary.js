...
export function adaptV1ToMercuryShape(snapshot, id) {
  ...
  return {
    ...
    // Stays are stored per city; flatten them with their city so they can be
    // listed in one section under the day-by-day, like the transfers.
    v1_stays: (snapshot.cities || []).flatMap((entry, index) =>
      (entry?.hotels || [])
        .filter((hotel) => hotel?.name)
        .map((hotel) => ({ ...hotel, city_name: cityLabel(entry?.city, index) })),
    ),
  };
}
