...
const buildTravel = (booking, { originCityName, destCityName }) => {
  ...
  return {
    ...
    modeLabel: transferModeLabel(booking?.booking_type),
    // First-token split — a combo reads "Taxi, Flight" and must not render a plane.
...
export function buildTripViewModel(state) {
  ...
  const legs = cities.map((c, i) => {
    ...
    const travel = buildTravel(inboundBooking, {
      originCityName: i === 0 ? I.start_city?.name : prev?.city?.name,
  ...
  // the category name and buries the real type under `.detail`.
  const anc = C?.summary?.Ancillaries?.bookings || A;
  const ancillaries = {
    visaCount: anc.filter((b) => b?.booking_type === "Visa").length,
    esimCount: anc.filter((b) => b?.booking_type === "eSIM").length,
    items: anc,
  };
