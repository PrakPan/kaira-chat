  ...
  const legs = cities.map((c, i) => {
    ...
    const stay = hotel
      ? {
          ...
          // score. That sheet draws the score properly under the photos — gold
          // stars, the number and the review count behind it — so a bare
          // "4.3★" a few pixels above it was the same fact stated twice, in
          // the weaker of the two ways, and a "★" that meant something
          // different from the one below it. The itinerary ROW still leads
          // with the score: there it is the only rating the traveller gets.
          detailMeta: [
