...
// Always production, in every environment — deliberately NOT
// NEXT_PUBLIC_MERCURY_HOST.
//
// The indexed-itinerary SEO layer is only populated on production Mercury. Dev
// Mercury answers the same endpoint with 660 rows whose `destination` and `url`
// are all null (production: 1,718 rows, none null). Next rejects a null route
// param, so a dev build fed from dev Mercury fails outright in getStaticPaths
// for /trips/[destination]/[slug].
//
// This used to work by accident: prebuild never saw the env files (dotenv read
// a `.env` that does not exist), so this always fell through to the production
// default. scripts/loadEnv.js made the env reach prebuild, which silently
// switched dev builds onto dev Mercury. Pinning it here makes the old behaviour
// an explicit decision instead of a side effect of broken env loading.
//
// Everything this module fetches is the public SEO projection (and itinerary
// bodies passed through toPublicTrip's allowlist), identical in content to what
// production serves — so staging rendering production trips is correct.
const MERCURY_HOST = "https://mercury.tarzanway.com";

// Re-exported, not redeclared: this used to be a second hardcoded copy of the
// origin, which is how a codebase ends up with two answers to "what is this
// site called". lib/seo/siteOrigin.js owns it now. Kept as a named export here
// because pages/trips/* and lib/seo/tripsJsonLd.js already import it from this
// module.
const { SITE_ORIGIN } = require("./siteOrigin");
...
/**
 * The itinerary's photo gallery — the same endpoint the live itinerary page
 * uses, and the only source that has every picture the trip actually carries
 * (43 for a ten-day Europe trip, against the 5 the SEO payload's city and
 * activity keys yield). PII-bearing, like the itinerary itself: run it through
 * toPublicGallery before it goes anywhere near props.
 */
async function fetchItineraryGallery(id) {
  return getJson(
    `${MERCURY_HOST}/api/v1/itinerary/${encodeURIComponent(id)}/gallery/`,
  );
}

/**
 * The itinerary's transfer bookings — flights, trains, buses and taxis between
 * (and within) its cities. The itinerary body above doesn't carry the
 * between-city legs at all; the live itinerary loads them from this endpoint.
 * Booking data (fares, PNR-adjacent codes, pax, dates): run it through
 * toPublicTransfers before it goes anywhere near props.
 */
async function fetchItineraryTransfers(id) {
  return getJson(
    `${MERCURY_HOST}/api/v1/itinerary/${encodeURIComponent(id)}/bookings/transfers/`,
  );
}
...
/**
 * The public projection of /api/v1/itinerary/<id>/bookings/transfers/.
 *
 * Upstream groups legs as `{ intercity, airport, intracity }`, each a map keyed
 * by route edge (or city) whose values are a booking or a list of them. The
 * intercity map is already in route order — home → first stop → … → home — so
 * insertion order is the trip's order and no date is needed to sort on.
 *
 * Kept: the leg's display name ("Train from Tokyo to Fujikawaguchiko"), its
 * mode(s) ("Flight", or "Bus, Train" for a combo leg) and the coarse duration
 * band ("1-2 hours"). Everything else — booking ids, fares and fare rules,
 * flight numbers, check-in/out dates, pax counts, vendor — is dropped by not
 * being named, the same allowlist rule as the itinerary body.
 */
const publicTransfer = (booking) => {
  if (!booking || typeof booking !== "object") return null;

  const name = str(booking.name);
  if (!name) return null;

  return {
    name,
    booking_type: str(booking.booking_type),
    travel_duration: str(booking.duration),
  };
};

function toPublicTransfers(raw) {
  if (!raw || typeof raw !== "object") return [];

  const legs = [];
  for (const group of ["intercity", "airport", "intracity"]) {
    const bucket = raw[group];
    if (!bucket || typeof bucket !== "object") continue;

    for (const value of Object.values(bucket)) {
      legs.push(...cleanList(Array.isArray(value) ? value : [value], publicTransfer));
    }
  }
  return legs;
}
...
// A gallery caption is "Zurich, March 3, 2026 - March 4, 2026" — the place,
// then the dates that one customer's booking ran for. These pages are evergreen
// and carry no calendar dates anywhere else (see the header of
// components/trips/TripSeoPage), so the range comes off and the place stays.
// Mercury writes ", None" where a POI has no date; that goes too.
const galleryCaption = (caption) => {
...
/**
 * The public projection of /api/v1/itinerary/<id>/gallery/.
 *
 * Allowlist, like toPublicTrip: only the media key, its kind and the cleaned
 * caption survive — `source` (which names the supplier) and anything the
 * endpoint grows later are dropped because they are not named here.
 *
 * Deduped on the media key. A city the route returns to appears once per visit
 * upstream, which is right for a trip log and wrong for a page of photos: the
 * old strip showed Zurich three times out of five pictures.
 */
function toPublicGallery(raw) {
